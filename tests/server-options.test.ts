...
it("worker options は宣言された registryMode を運ぶ(ADR 0052 / ADR 0043)", async () => {
  ...
  const options = buildWorkerOptions(
    ...
    { db: openDb(":memory:"), clock, ...containerHarness(fakeContainers()), onCapInterrupted: () => {}, onSpawnFailed: () => {}, onWorkerExited: () => {} },
  );
...
it("ClaudeWorkerOptions の任意フィールドは、テスト用の注入 seam を除いて全て組み立てられる(ADR 0043)", async () => {
  ...
  const emitted = new Set(
    Object.keys(buildWorkerOptions({ ...composition(), registryDir }, { db: openDb(":memory:"), clock: new FakeClock(), ...containerHarness(fakeContainers()), onCapInterrupted: () => {}, onSpawnFailed: () => {}, onWorkerExited: () => {} })),
  );
...
it("kill switch は盤面の合成からそのまま worker options へ届く(判断8)", async () => {
  ...
  const options = (advisorDisabled: boolean) =>
    buildWorkerOptions(
      ...
      { db: openDb(":memory:"), clock: new FakeClock(), ...containerHarness(fakeContainers()), onCapInterrupted: () => {}, onSpawnFailed: () => {}, onWorkerExited: () => {} },
    );
...
it("worker ログの置き場は、盤面が守っているパスと同じ1つである(ADR 0040)", async () => {
  ...
  const options = buildWorkerOptions(board, { db: openDb(":memory:"), clock: new FakeClock(), ...containerHarness(fakeContainers()), onCapInterrupted: () => {}, onSpawnFailed: () => {}, onWorkerExited: () => {} });
