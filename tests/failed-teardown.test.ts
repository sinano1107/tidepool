...
import { FAILED_TEARDOWN_QUESTION_TITLE, isQuarantineOpen } from "../src/quarantine.js";
...
const openQuestion = (f: Fixture) => getTask(f.db, isQuarantineOpen(f.db, "failedTeardown", f.task.id)!.id)!;
...
it("受理の検査が投げれば回答が拒まれ、question は開いたまま、例外本文がメッセージに載る", async () => {
  ...
  expect(isQuarantineOpen(f.db, "failedTeardown", f.task.id)?.id).toBe(question.id);
