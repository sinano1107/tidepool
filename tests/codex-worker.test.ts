...
describe("CodexWorker (ADR 0098)", () => {
  ...
  it("見える approved の記憶があれば注入節を taskPrompt の先頭に置き、worker_spawned の直後に memory_injected を書く。無ければ節を置かず entries 空で残す(spec #586 C / issue #592)", async () => {
    const f = await fixture();
    ...
    const value = task(f.db, "codex-memory");
    f.worker.start(value);
    const { section } = buildMemoryInjection(f.db, value, "work", "codex-agent");
    expect(f.process.calls[1]!.args.at(-1)!.startsWith(`${section}\n\n`)).toBe(true);

    for (const [id, entries] of [[bare.id, []], [value.id, [{}]]] as const) {
      const events = listEvents(f.db, id);
