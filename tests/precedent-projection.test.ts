...
it("同じタスクの別 worker session の decision は、この Episode のマーカーにならない(issue #379: 1タスクに複数 session)", () => {
  ...
  const events: EventRow[] = [
    ...
    // 2本目の session — retry / 統合復帰 / quarantine 復帰のいずれでも同じ形
    {
      ...
      payload: { kind: "worker_spawned", registry_commit: "abc", definition_version: "0.1.1", advisor: null, provider: "anthropic", model: "opus", effort: "high", source: { tier: "board" }, harness: "claude-code", cli_version: "test" },
      created_at: at,
    },
    {
