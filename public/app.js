...
function QueueScreen({ data, paused, onTogglePause, spendDown, onSpendDown, onFront, onDoneHuman, onReorder }) {
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { padding: "20px 16px" } }, /* @__PURE__ */ React.createElement("h1", { style: { fontSize: "var(--text-xl)", margin: "0 0 2px" } }, "Queue"), /* @__PURE__ */ React.createElement("p", { style: { fontSize: "var(--text-sm)", color: "var(--text-secondary)", margin: "0 0 16px" } }, "FIFO \xB7 new tasks append \xB7 reorder never resets \xB7 concurrency=1"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, marginBottom: 10, minHeight: 30 } }, /* @__PURE__ */ React.createElement("span", { style: { fontFamily: "var(--font-mono)", fontSize: "var(--text-2xs)", color: slot.color, textTransform: "uppercase", letterSpacing: "0.08em" } }, "slot"), slot.taskId && /* @__PURE__ */ React.createElement(IdChip, { id: slot.taskId, style: { fontFamily: "var(--font-mono)", fontSize: "var(--text-2xs)", color: "var(--text-muted)", flexShrink: 0 } }), /* @__PURE__ */ React.createElement("span", { style: { flex: 1, minWidth: 0, fontSize: "var(--text-sm)", color: !paused && !data.running ? "var(--text-muted)" : "var(--text-body)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, slot.line), /* @__PURE__ */ React.createElement(
    ...
    const spend = spendDown[usage.provider]?.[window2.window];
    return /* @__PURE__ */ React.createElement("div", { key: `${window2.window}:${window2.model ?? ""}`, style: { display: "flex", alignItems: "center", gap: 8, marginTop: 3, fontFamily: "var(--font-mono)", fontSize: "var(--text-2xs)", color: window2.throttled ? "var(--coral-4)" : spend ? "var(--text-body)" : "var(--text-muted)" } }, /* @__PURE__ */ React.createElement("span", { style: { flex: 1, minWidth: 0 } }, window2.window, window2.model ? ` \xB7 ${window2.model}` : "", " \xB7 ", window2.usedPercent ?? "?", "% \xB7 ", spend ? "spend-down \xB7 100% cap \xB7 expires at reset" : `offset ${window2.offset}pt \xB7 ${window2.throttled ? `paced${window2.resumesAt ? ` until ${new Date(window2.resumesAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : ""}` : "on pace"}`), spend !== void 0 && /* @__PURE__ */ React.createElement(Button, { variant: "secondary", size: "sm", "aria-label": `${spend ? "cancel spend-down" : "spend down"} ${usage.provider} ${window2.window}`, onClick: () => onSpendDown(usage.provider, window2.window, !spend) }, spend ? "cancel" : "spend down"));
...
function App() {
  ...
  const setSpendDown = async (provider, window2, active) => {
    try {
      await api("/api/spend-down", { provider, window: window2, active });
      await refresh();
      say(
        active ? "warn" : "info",
        active ? `spend-down armed \xB7 ${provider} ${window2}` : `spend-down cancelled \xB7 ${provider} ${window2}`,
  ...
  const register = async (fields) => {
    try {
      ...
      if (!(err instanceof ApiError) || err.status !== 422) say("danger", "registration failed", String(err.message || err));
      throw err;
    }
  };
  const openTask = (t) => {
