 ...
 *  entry ごとに1度だけ Board call を回し、RCA の findings を証拠に確定させた cause を
 *  同じ entry への新しい event(round = after_rca)として追記する。
 *
 *  1度だけ、の門は「決着したのが RCA 子自身で、それで全部が揃った」こと —— 後から同じ
 *  タスクの統合 review が決着しても、異議されたタスクごと cancel されても(扉には根が
 *  渡る)ここは撃たない。撃てない・失敗した entry には何も書かず `uncertain` のまま残る
 *  (第3回は無い)。同じタスクに後日また異議が束ねられて新しい RCA 子が立てば、その
 *  決着でこの門はもう一度開く —— それは新しい RCA 群の後の第2回であって第3回ではない。
 *  settlement 経路の後に fire-and-forget で呼ぶ: ここからは投げない。 */
  ...
  await Promise.all(
    pending.map(async (initial) => {
      ...
      const input: AttributionInput = {
        ...
        steering: objection_event_ids.map((id) => {
      ...
      try {
        ...
        console.error(`[attribution] ${objectedId} entry ${entry_id}: ${message(err)}`);
      }
    }),
  );
}
