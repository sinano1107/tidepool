...
export async function submitAnswer(
  ...
  // (pin 不一致の DomainError は回答ごと巻き戻す)、reject は同じ pin 検査の後に candidate を `rejected` で無効化(invalidate op の reject は何もしない)
  const { question, parentUnblocked, pickupResumed } = deps.db.transaction(() => {
    ...
    if (proposal && answers[0] === "approve") approveMemoryProposal(deps.db, proposal, task.id, origin, now());
      ...
      if (proposal.op !== "invalidate") invalidateMemoryEntry(deps.db, { entry_id: proposal.candidate_id, reason: "rejected" }, HUMAN_WORKER_ID, origin, now());
    }
    return answered;
  })();
  if (wantsMerge) {
