...
import { settleStaleProposals } from "./execution-setting.js";
...
export function registerDueMetaReviews(db: Db, now: Date, agents?: () => readonly { name: string; tier?: string }[]): void {
  ...
  for (const subject of Object.keys(META_REVIEW_SUBJECTS) as MetaReviewSubject[]) {
    ...
    if (subject === "routing" && agents) {
      try {
        settleStaleProposals(db, now, null, agents);
      } catch (err) {
        // registry が読めなくても poll(pickup)は止めない —— registry の pin はこの poll では照合しないだけ
        console.warn(`[meta-review] registry pins not checked: ${String(err)}`);
      }
    }
