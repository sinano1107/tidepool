...
9. **スロットルは ADR 0030 の pacing であって上限ではない。** pace offset(`provider_pace_offsets`)と spend-down で開ける。**spend-down は対象ウィンドウのリセットまで自動失効しない**ので、実験を中断するときは手で解除する。**オフセットは 0〜100 の減算のみ**なので「盤面が線形ペースを X pt 先行してよい」というツマミは存在しない —— 0 が最も緩い設定であり、バースト形状の仕事は構造的に自分を止める。一気に流したいときは spend-down が唯一の逃し弁である。
