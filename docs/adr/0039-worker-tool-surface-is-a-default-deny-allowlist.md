...
**追記(2026-08-04)**: Pi の CLI を **2.1.221** に更新し、両ホストが同版になった(advisor の
エイリアス `opus` を Opus 4.8 → Opus 5 に解決させるため。issue #33)。更新後に canary 3本を
Pi で再実行し、containment / settings-floor / tool-floor のいずれも従前と同一の形で PASS
した —— 上の 2.1.207 の数字は当時の実測として残る。版差が消えた今もこの control が要る
理由は変わらない: 次にどちらか一方のホストだけが更新された日に、同じ問いをもう一度
立てることになる。なお `--model sonnet` の解決先は両版とも `claude-sonnet-5` で、更新は
worker のベースモデルを動かしていない(更新前に実測して確認)。
