...
**Status 追記: ADR-0110 でティアの水準だけは盤面が judge する** —— advisor の綴りが自由文字列(model 名)から**真偽値 + 表からの導出**に変わり、盤面は「advisor のティアが main 以上か」を spawn 前に検査するようになった。本 ADR が却下したのは**盤面が意味を所有していない文字列**(ホストの CLI 版で解決先が動く alias)を表で judge することであり、その論拠はここには届かない: 判定に使うのは盤面が自分で組んだ表のティア行の順序だけで、alias が実際にどのモデルへ解決されるかは今も judge しない。事後の観測(`worker_exited.usage.advisor`)を唯一の防壁とする本 ADR の結論も、その部分は不変である —— 2026-09-10 の実測で、未 attach は stream-json に一切通知が出ないことが確かめられた(pairing 不成立でも exit 0、stderr に警告1行のみ)。
...
残していた(禁じれば advisor の `usage` が常に分離できる)。しかし禁じられるのは**文字列の
一致**だけであり、`main: sonnet` × `advisor: claude-sonnet-5` は文字列としては
異なるまま同じモデルに解決される。分離不能なケースは残るので `usage: null` の枝は
...
- **advisor == main を禁じる** — 文字列一致でしか禁じられず、エイリアスと具体 id の
  組み合わせをすり抜ける。`usage: null` の枝を消せないので、何も買わない。
