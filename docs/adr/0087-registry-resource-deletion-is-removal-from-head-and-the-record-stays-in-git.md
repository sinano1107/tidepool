...
3. **消せない資源**: 盤面自身の registry clone(保護剥奪と同じく永久に拒む)、盤面の既定 agent と既定
   ...
   **訂正(2026-08-25 / issue #376)**: この3つは列挙ではなく**規則**として読む —— 消せないのは
   「盤面のポインタが指す先すべて」であり、盤面の第3のポインタである Auditor(CONTEXT.md「Auditor」、
   既定名 `fugu` — ADR 0089)も入る。Auditor の解決失敗は quarantine が受け止めるので決定2の profile
   の穴とは違うが、扉が意図的に quarantine を作ることの根拠には決定2自身がしていない。起票時は実
   registry に auditor が無く縁のケースだったが、種まき(ADR 0089 / issue #365)が auditor を必ず
   載せるようになったので、門は新規盤面すべてに効く。
...
決定3 の規則に例外が1つ — 組み込み agent(ADR 0117)を shadow している同名エントリは消せる。消えても組み込みに落ちるだけで、
ポインタの指す先が失われる事故にならない。
