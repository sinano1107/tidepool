...
## 改訂注記(2026-08-25、issue #473 / ADR 0100)

「本人の Mac 上の盤面」は macOS の盤面を意味しない。#465 で macOS の worker 容器が fail-closed 確定したため、Mac の
盤面は Mac 上の Linux VM の中で動かす(ADR 0100)。決定1 は VM 内で本人が `/login` した CLI に、決定2 は VM 内で盤面を
起動するホスト側コマンドを `caffeinate` で包む形に、決定3 は VM 内の home(DB / worker-logs は VM ネイティブディスク)
に読み替える。決定4・5 は不変。
