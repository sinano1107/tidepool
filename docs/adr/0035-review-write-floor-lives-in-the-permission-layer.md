...
**追記(#146 で解決)**: 原因は読み取り床ではなく、サンドボックスのネットワーク既定が loopback への `listen` を拒否していたことだった(review 固有でもなく work プロファイルでも同じく落ちる)。両プロファイルの `network: { allowLocalBinding: true }` で 858 tests 全 green になっている — 詳細は ADR 0033 の #146 追記。registry への値の投入は依然この決定の範囲外(ホストごとの登録判断)である。
