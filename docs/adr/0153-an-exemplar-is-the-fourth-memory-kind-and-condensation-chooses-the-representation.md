...
3. **worker が読む本文は 注釈 + case 描画(decision 本文 + steering + Episode の handoff / result)。** transcript は盤面側のまま
   ...
   持ちそこから Episode まで辿れるのに worker には数字しか返らない、既存経路の穴であり観測を待たない。anchor は描画時に検証し、
   一致しない注釈は text を残して anchor を未解決と印し、件数を pull の event に刻む(周期検査の項目にはしない — 描画は不変の
   記録の派生で、外れるのは描画器の版が変わったときだけ)。
...
- **anchor をオフセット / 段落番号で持つ** — 描画の書式に結合する。
