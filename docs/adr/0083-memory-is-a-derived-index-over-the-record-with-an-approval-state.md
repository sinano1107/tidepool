...
## 追記 6(2026-09-15 の grilling、issue #604 — #606 で関連の当たりが締まった後に)

**spawn 注入の関連 leaf は本文を運ばず、title + path + 出所の種別のポインタを関連度の順位で並べる。本文を読むのは
`read_memory` の pull だけ。** 追記4 の「関連 leaf は今の task に効く事実を運ぶ」はこの1文だけ改訂し、「関連 leaf を
ゼロにして全部 pull させる案」の却下(決定10 の「関連度で注入」)は維持する — 順位つきの一覧が注入である。理由はコストでは
ない(#606 後の当たりは数件で、本文込みとの差は数十トークン)。(1) 本文を読む行為が必ず pull になり、Episode の
「D の前に読んだ記憶」が機械記録どおりの意味を持つ — RCA が「pull すらしなかった」と「読んで従わなかった」を分けられる。
本文注入では両方が「文脈にあった」に潰れる。(2) 決定12 が skill 本文の注入を退けた線(progressive disclosure を壊し、
ポインタと二重になる)に leaf も揃う。「title を見ても pull しない worker には事実が届かない」は、この形では観測できる
失敗であり、本文注入の形では観測できない失敗である。

**削り順は 関連 leaf を順位の下から1件ずつ → INDEX を深い階層から浅く → 最上位 INDEX は超えても残す。** 追記4 の
「本文 → 深さ → leaf」から本文の段が消え、残り2段は入れ替える。leaf 行と INDEX 行はいずれもポインタで同程度のコストになり、
違いは出自だけになった — INDEX 行は書き手が宣言した不変の定義、leaf の下位は OR-FTS の順位の裾で注入の中で最もノイズに
近い。超過の現実的な経路は当たりの多さであって枝の深さではなく、そのとき宣言済みの地図を削って推測の裾を残す理由が無い。
「注入の本体は Definition つき INDEX」(追記4)に削り順を揃える形でもある。落ちた枝は `browse_memory` で決定論的に
降りられ、落ちた leaf の裾は失っても小さい。件数上限・順位の閾値は置かない — 上限トークンが唯一のノブ(決定10)で、
行が等長になった今は件数上限は単位を変えた同じノブであり、裾の品質を直すのは閾値だが FTS の rank は query 間で較正されて
いない。当たりの件数と裾の質は未観測(派生 issue #612)。

**記憶との接触は seen と read の2語に分ける。** seen = ポインタを見た(注入の目次行・関連 leaf 行、browse / search が
返した id)、read = 本文を読んだ(`read_memory` が返した id)、seen ⊇ read。「D の前に読んだ記憶」(spec #586 D)は read
だけを数え、seen は隣の列に持つ。従来の派生は pull の verb を見ず title を見ただけの id も「読んだ」に数えていた。
