...
これはツール面の門であって credential の床ではない。管理MCP は人間面のリスナーに mount され新しい credential を
作らない(ADR 0032)ので、その credential を持つ対話エージェントが `/api` を直接叩けば `confirm: true` は
立てられる。今回の線が塞ぐのは「MCP ツールとして自然に着地する経路」であり、`/api` 直叩きは注入されたテキストが
credential の所在まで知る別段の逸脱として、観測されるまで受容する。
