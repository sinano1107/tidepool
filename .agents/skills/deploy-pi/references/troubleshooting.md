...
- **A startup modal is blocking the prompt.** The `Try "` placeholder marker never appears and `checkUsage` times out → null. Two distinct gates cause this:
  - **The folder-trust gate** ("Is this a project you trust?") on a fresh/re-cloned board cwd. Run the seed (issue #442) — no interactive session needed: `ssh masaki@100.78.52.97 'node /opt/tidepool/scripts/seed-claude-trust.mjs /opt/tidepool'`. It merges `projects["/opt/tidepool"].hasTrustDialogAccepted: true` into `~/.claude.json` (home-side, survives redeploy) without touching anything else in the file.
  - **A what's-new/onboarding modal** after a `claude` update. This gate is separate and still needs a one-time interactive dismissal, as masaki: `ssh masaki@100.78.52.97`, then `cd /opt/tidepool && claude` — dismiss the modal, then quit. It persists until the next CLI update.
