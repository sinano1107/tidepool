...
**Also prepare the board's Claude CLI config** (issue #81 / ADR-0028, issue #442, issue #682): the hourly usage throttle scrapes `/usage` from an *interactive* `claude --safe-mode` session run in `/opt/tidepool`, and the interactive TUI blocks the input prompt behind its first-run dialogs — the folder-trust gate ("Is this a project you trust?"), and on a fresh install the onboarding flow (theme, then login method). After the `gh auth login` step above, run the script over ssh — it arrives with the deploy, so no separate copy step is needed:
