...
# 3a. work profile: filesystem controls, loopback bind and allowed-domain reach
#     all share one emitted profile/session.
...
**3a(6) is ADR 0072's row.** It proves `allowedDomains` was accepted and used, and is meaningful only alongside rows (1)/(4), which prove the settings file was not silently dropped. That hosts outside the allowlist stay closed — the tailnet names included — is the containment canary's question below, not this row's.
...
**Exit codes: `0`** every row measured and refused — **`1`** something got out (a hook fired, a settings write landed, an unhonoured rule, *or the sandbox failed to start*) — **`2`** nothing got out but a row could not be measured. Unlike the containment canary, `2` is **not** a steady state here: every row should pass on both hosts.
...
`board_hook_live_verdict()`, `control_verdict()`, `deny_verdict()`, `scope_verdict()` and `project_hook_verdict()` are where "not measured" could quietly become "measured and fine"; `scripts/hook-canary.test.sh` pins every branch of all five (`bash scripts/hook-canary.test.sh`, no Pi and no session needed).
