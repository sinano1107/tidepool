...
worker がタスクの途中で上位モデルに判断の相談をするオプション能力。advisor は助言のみを返し、決して行動しない — 助言を踏まえた判断はあくまで worker 本人の判断として decision log と完了時レビューに全部吸収される。Subagent と同じく説明責任を分割しない側(ADR 0010 の線)であり、権限内判断の質を高めるが決裁権の境界は決して広げない。エージェント単位の opt-in(フィールド不在 = 無効)だが、運用上は原則すべてのエージェントに付与し、外したい特殊ケースだけ無効化する — 手戻り(fix-forward)はトークンと人間の時間の両方を消費するため、判断点でのみ発火する advisor のコストのほうが安い。有効・無効の正本は registry(agent.md)であり、切り替えは registry への書き込み — 盤面側のオーバーライドは持たない。各セッションの実効 advisor 構成は機械記録され、構成の帰属は常にイベント履歴から確定できる。advisor への相談を前置きとして課せるのは権限内の迷いに対してのみ — 決裁権外の判断のエスカレーションには一切の前置きを課さない(安全弁は無摩擦のまま)。相談の記録は機械観測のみで、自己申告には依らない。
