...
タスクを実行する worker への割当。登録時は任意であり、指定した場合は委譲先の要求として登録者の決裁権(assignable_to)に対して検査される。未指定は「未割当で待つ」状態ではなく**その時の盤面の既定 agent への参照**であり、pickup の瞬間に解決される(値は焼き込まれない — Workspace の「既定への参照」と同型)。したがって割当待ちという第3の停止状態は存在しない — slot に入らない理由は blocked(下からの塞ぎ)と held(上からの凍結)だけである。
