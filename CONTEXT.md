...
`todo` / `in_progress` / `blocked` / `done` / `cancelled` の5つ。`skipped` はステータスではなく `todo` 上の表示用モディファイア(キュービューのみ)— タスク自身にもキュー順序にも問題はないが、盤面側の事情(環境事象 = throttle・実行 workspace の quarantine、または人間の Pause)が今は slot に入れさせないことを示す。`held` もステータスではなく導出される表示状態(Held 参照)。
...
worker がタスクの途中で外部モデル(external model)に執筆・生成を発注し、成果物を検収して自分の成果として着地させるオプション能力(2026-07-27 の grilling、ADR 0032)。Subagent・Advisor と同じ「説明責任を分割しない側」(ADR 0010 の線)の第3項であり、外注先は盤面上の身元を持たない — Worker ではなく worker session 内の道具で、workspace・branch discipline・decision log・escalation のいずれにも触れない。命名もこの線に従う: 外注先に住人風の名(海の生き物)は与えない — 身元を持つ者の命名伝統を身元なき道具に使わない。「上司AI/部下AI」の対も採らない(発注者は普通の worker)。

門は2つの AND: agent 単位の opt-in(フィールド不在 = 無効、正本は registry — Advisor と同型)∧ タスクの workspace の外部送信可(external-send、Workspace 参照)。両方成立するセッションにだけ外注の道具が接続され、条件外のセッションには**道具そのものが存在しない**(instructions への信頼ではなく構造で強制 — 保護ブランチの構造的禁止と同じ流儀)。片側だけの状態は誤設定ではなく意図された絞り込みでありうる — 機械は意図を推測せず、spawn 時の実効外注状態(有効/どの門で無効か)を機械記録する(Advisor の実効構成記録と同型)。

- **面接(interview)**: 外注先を選ぶセッション内の使い捨て選抜。同一課題を複数候補に送り、発注者が読み比べて選ぶ(候補の自己採点は使わない)。順位表はセッションを超えて持ち越さない(永続化は痛みが観測されてから)。レビュー用途に面接はない — 独立レビューが外部モデルに求めるのは最良の1体ではなく系統の異なる複数の視点であり、選抜ではなく同報になる
- **検収と着地**: 発注した worker は成果物を全部読み(報告文は信じない)、workspace への適用・branch discipline・handoff doc という着地の責任を常に負う — 外注できるのは内容の著作であって責任ではない。不合格は差し戻しが既定(費用上の推奨)だが、自分で直すことは禁じない — worker 本人の作業として常に正当。外注するつもりで道具がなければ、黙って自前実行に切り替えず decision log に1行残す
- 送信後のデータの扱い(保持・学習利用)は盤面の関知外 — アカウント設定が担う二層目で、盤面が持つ線は「送信の可否」のみ。従量費も v1 では盤面の観測外 — 損害上限は盤面保有の単一名義キーの側で構造的に閉じる(ADR 0032)
...
エージェントワーカーの spawn から exit までの1回の実行。タスクと多対1 — 1つのタスクは retry や統合復帰により複数の worker session を持ちうる。終了時にトークン消費の内訳と推定ドル(実行時点の API 換算推定値 — サブスクリプション下では実請求額は存在しない)が記録される。kill などで終了報告が欠けたセッションは欠測(null)として記録され、事実が消えることはない。spawn 時に読まれた registry main の commit hash も記録される — 「どの版の定義で走ったか」は後から導出できない歴史的事実の観測であり、焼き込みではない(ADR 0020)。Throttle の「使用量(Usage)」はアカウント単位のウィンドウ使用率(%)であり、こちらはセッション単位のトークン消費 — 別概念。
...
**外部送信可(external-send)**: workspace の内容を外部モデルプロバイダーへ送信してよいことを宣言する、感度を資源側に張り付けるマーカー(protected と同族、2026-07-27 の grilling)。既定は送信禁止 — 外部送信は不可逆であり fail-closed 側に倒す。宣言のない workspace のタスクには外注の道具が接続されない(外注 参照)。
