...
軌道修正の判断点は統合復帰の1回だけであり、子1つの完了ごとに親を呼び出す checkpoint 機構は存在しない(ADR 0045、2026-08-04 の grilling、issue #171)。代わりに文脈が流れる: 子は spawn 時の文脈として決着済みの兄弟の成果(handoff doc)と親の判断ログを受け取り、先行結果と自分の前提の食い違いに気づいた子は escalation で人間を呼ぶ。親のレンズ(agent 定義)にしか無い判断材料に計画の正否を依存させてはならない — 実行に効く制約・意図は decompose 時に子の purpose と分解理由へ**外化**するのが作法であり、外化の失敗は fix-forward が拾う。
...
work タスク完了時に必須の引き継ぎ文書。行動可能な情報はタスクになり、doc は文脈のみを運ぶ。読み手は統合復帰する親・後続の兄弟・統合点の完了時レビュー(ADR 0045)。question / review タスクには不要。
