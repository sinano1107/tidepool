...
タスクが終端に達した状態の総称(`done` または `cancelled`)。独立したステータスではなく2値の言い換え。決着した子は親を塞がない(blocked の導出上の「完了扱い」)。ルートが決着したツリーはボードから退く(Board 参照)。

## Board(ボード)

生きている計画を映すビュー。ルートが settled になったツリーは配下ごと視界から退き、cancelled のタスクは計画が生きていても即時退く(破棄された計画の残骸を再計画の視界に置かない)。done の子はルートが決着するまで計画の進捗として見え続ける。退くのは視界からのみで、保存される状態の変化ではない(純粋な導出)— タスクは個別に参照でき、decision log は不滅で、記録は何も消えない。
